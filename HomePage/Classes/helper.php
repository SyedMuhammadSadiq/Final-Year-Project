...
/* change requirement 3rd milestone    public function ModifyApptDate($params){
        $result = $this->update("appointments",array("Dates" => $params['prefferedDate'],"confirm" => 1),"where id = ?",array($params['AppointmentId']));
        return $result;
    }*/
    ...
    public function Pactivity($params){
        ...
        $result = $this->customSelect('select p.patientid,d.fname,d.lname,d.specialityid,s.name,a.Dates from appointments as a inner join
patient as p on p.patientid = a.patientid inner join doctor as d on d.id = a.doctorid left join
speciality as s on s.id = d.specialityid where a.confirm = 1 and p.patientid = '.$params['patientID']);
        return $result;
    }
...
/* 29/5/2017    public function Dappointment($params){
        $pdo = new pdocrudhandler();
        $query = "select p.fname,p.lname,p.nic,p.phone,a.description,a.Dates from appointments as a inner join
                  patient as p on p.patientid = a.patientid inner join doctor as d on d.id = a.doctorid
                  where a.confirm = 1 and d.id = ".$params['doctorId'];
        $result = $pdo->customSelect($query);
        return $result;
    }*/
    ...
    public function doctorsignup($params){
        ...
        if (empty($missingFeild)){
                unset($params['city']);
                $response = $this->insert('user',array('email' => $params['username'],'password' => $params['password'],'active' => 1,'lastLogin' => date('Y-m-d h:i:s'),'professionid' => 1));
                if($response['status'] == "success" && $response['rowsAffected'] == 1){
                    $get = $this->select('user',array("id"),"where email = ? and password = ?",array($params['username'],$params['password']));
                    $id = $get['result'][0]->id;
                    if ($id){
                        $result = $this->insert('doctor', array('fname' => $params['fname'], 'lname' => $params['lname'], 'email' => $params['email'], 'gender' => $params['gender'], 'address' => $params['address'], 'nic' => $params['nic'], 'phone' => $params['phone'], 'pmdc' => $params['pmdc'], 'degree' => $params['degree'], 'board' => $params['board'], 'fees' => $params['fees'], 'areaid' => $params['areaid'], 'specialityid' => $params['speciality'], 'userid' => $id));
                        $result['userid'] = $id;
                        return $result;
                    }
                }
...
if(isset($_POST['call']) && isset($mendetoryParam[$_POST['call']])){
    ...
    if(count($missingFields) > 0){
        ...
        $data = $_POST;
        echo json_encode($helperObj->$_POST['call']($data));
    }
}elseif (isset($_POST['call']) && $_POST['call'] == 'doctorsignupTimetable'){
    $helperObj = new helper();
    echo json_encode($helperObj->$_POST['call']($_POST));
}else{
